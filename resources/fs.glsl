...
if(oldval==0)
	...
	if(current_array_pos<MSIZE)
		{
		imageStore(img_geo, ivec2(current_array_pos,0), vec4(vertex_pos,0.0));
		imageStore(img_geo, ivec2(current_array_pos,1), vec4(gl_FragCoord.xy,current_array_pos,0));
		imageStore(img_geo, ivec2(current_array_pos,2),  vec4(vertex_normal,0));
		}		
