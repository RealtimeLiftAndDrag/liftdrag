...
void main() {
    ...
    if (oldval == 0) {
        ...
        if (current_array_pos < ESTIMATEMAXGEOPIXELSSUM) {
            imageStore(img_geo, store_geo(current_array_pos, 0), vec4(vertex_pos, 0.0f));
            imageStore(img_geo, store_geo(current_array_pos, 1), vec4(gl_FragCoord.xy, current_array_pos, 0.0f));
            imageStore(img_geo, store_geo(current_array_pos, 2), vec4(vertex_normal, 0.0f));
        }		
