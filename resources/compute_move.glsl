...
void main() 
	...
	for(int ii=0;ii<cs_workload_per_shader;ii++)
		...
		vec2 ntexpos = texpos - direction_result.xy*8.5;
		col = imageLoad(img_FBO,ivec2(ntexpos));
		if(col.b < 0.1 && col.g < 0.1 && col.r < 0.1)
			{			
			uint current_array_pos = atomicAdd(geopix.out_count[counterswap],1);
			if(current_array_pos>=ESTIMATEMAXOUTLINEPIXELS)
				break;
			geopix.out_texpos[counterswap][current_array_pos] = vec4(ntexpos,0,0);
			geopix.out_momentum[counterswap][current_array_pos] = vec4(direction_result,0);
			}
